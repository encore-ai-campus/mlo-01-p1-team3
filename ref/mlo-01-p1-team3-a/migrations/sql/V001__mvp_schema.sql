...
-- The API object contains repeated reference entities inside each listing.
-- Keep those entities once, then reference them from the current listing
-- projection. Listing-specific facts remain at listing grain for the MVP.
-- A listing reaches its brand through model -> brand; do not duplicate that
-- relationship on the listing row. Business-area parents use a self-FK and
-- are joined when a parent name is needed.
...
-- The registration report is stored at measure grain. Each API formList row
-- is flattened into (month, province, district, vehicle type, usage type,
-- quantity) before loading.

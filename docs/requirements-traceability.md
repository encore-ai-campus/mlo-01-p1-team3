...
> review_evidence: `evidence/requirements-review.md`
...
- 폐기된 ID를 재사용하지 않는다.
- 의미가 변경되면 `docs/change-log.md`에 변경 사유와 영향 범위를 기록한다.
...
- 아래 PRD/AC ID는 **프로젝트에 맞춘 후보 ID**이며 `docs/prd.md` baseline 때 최종 확정한다.
...
> 아래 ID는 추적표 작성을 위한 **초안**이다. 최종 `docs/prd.md`가 다른 식별자를 채택하면 PRD를 정본으로 하고 이 문서의 ID를 동기화한다.
...
3. `docs/prd.md`에서 최종 FR/DR/NFR ID를 확정하고 후보 ID와 동기화한다.
